...
describe "Normalize: case" do
  it "normalizes case with call" do
    assert_expand "case x; when 1; 'b'; when 2; 'c'; else; 'd'; end", "__temp_1 = x\nif 1 === __temp_1; 'b'; else; if 2 === __temp_1; 'c'; else; 'd'; end; end"
  end

  it "normalizes case with var in cond" do
    assert_expand_second "x = 1; case x; when 1; 'b'; end", "'b' if 1 === x"
  end

  it "normalizes case with Path to is_a?" do
    assert_expand_second "x = 1; case x; when Foo; 'b'; end", "'b' if x.is_a?(Foo)"
  end

  it "normalizes case with generic to is_a?" do
    assert_expand_second "x = 1; case x; when Foo(T); 'b'; end", "'b' if x.is_a?(Foo(T))"
  end

  it "normalizes case with Path.class to is_a?" do
    assert_expand_second "x = 1; case x; when Foo.class; 'b'; end", "'b' if x.is_a?(Foo.class)"
  end

  it "normalizes case with Generic.class to is_a?" do
    assert_expand_second "x = 1; case x; when Foo(T).class; 'b'; end", "'b' if x.is_a?(Foo(T).class)"
  end

  it "normalizes case with many expressions in when" do
    assert_expand_second "x = 1; case x; when 1, 2; 'b'; end", "'b' if (1 === x) || (2 === x)"
  end
  ...
  it "normalizes case without value" do
    assert_expand "case when 2; 3; when 4; 5; end", "if 2; 3; else; 5 if 4; end"
  end

  it "normalizes case without value with many expressions in when" do
    assert_expand "case when 2, 9; 3; when 4; 5; end", "if 2 || 9; 3; else; 5 if 4; end"
  end

  it "normalizes case with nil to is_a?" do
    assert_expand_second "x = 1; case x; when nil; 'b'; end", "'b' if x.is_a?(::Nil)"
  end

  it "normalizes case with multiple expressions" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when {2, 3}; 4; end", "4 if (2 === x) && (3 === y)"
  end

  it "normalizes case with multiple expressions and types" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when {Int32, Float64}; 4; end", "4 if x.is_a?(Int32) && y.is_a?(Float64)"
  end

  it "normalizes case with multiple expressions and implicit obj" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when {.foo, .bar}; 4; end", "4 if x.foo && y.bar"
  end

  it "normalizes case with multiple expressions and comma" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when {2, 3}, {4, 5}; 6; end", "6 if ((2 === x) && (3 === y)) || ((4 === x) && (5 === y))"
  end

  it "normalizes case with multiple expressions with underscore" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when {2, _}; 4; end", "4 if 2 === x"
  end

  it "normalizes case with multiple expressions with all underscores" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when {_, _}; 4; end", "4 if true"
  end

  it "normalizes case with multiple expressions with all underscores twice" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when {_, _}, {_, _}; 4; end", "4 if true"
  end

  it "normalizes case with multiple expressions and non-tuple" do
    assert_expand_second "x, y = 1, 2; case {x, y}; when 1; 4; end", "4 if 1 === {x, y}"
  end
  ...
  it "normalizes case with Path.class to is_a? (in)" do
    assert_expand_second "x = 1; case x; in Foo.class; 'b'; end", "if x.is_a?(Foo.class); 'b'; else; raise \"unreachable\"; end"
  end
end
