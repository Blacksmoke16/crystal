...
module LLVM
  module Coverage
    ...
    def self.instrument_function(func : Function)
      ...
      # Must start with * and have a simple function name
      return unless name.starts_with?("*")
      return unless name =~ /^\*[a-z_][a-z0-9_]*:/i
      
      # Create profile name variable
      ...
      # Count basic blocks first
      num_counters = 0_u32
      func.basic_blocks.each { num_counters += 1 }
      return if num_counters == 0 # Skip empty functions
      ...
      func.basic_blocks.each do |bb|
        ...
        # Position at start of basic block
